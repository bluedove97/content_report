"""
r01 모닝 리포트 수동 실행
사용법: python run_manual_r01.py --tickers "005380,035420" --emails "a@b.com,c@d.com"
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="r01 모닝 리포트 수동 실행")
    ...
    print(f"[manual_r01] 대상 종목: {[c['name'] for c in companies]}")
    print(f"[manual_r01] 수신 이메일: {args.emails}")
    ...
    try:
        ...
        print(f"[manual_r01] fetch_stock 실패:\n{traceback.format_exc()}", file=sys.stderr)
        failures.append("fetch_stock")

    try:
        fetch_news.run()
    except Exception:
        print(f"[manual_r01] fetch_news 실패:\n{traceback.format_exc()}", file=sys.stderr)
        failures.append("fetch_news")

    if "fetch_stock" in failures and "fetch_news" in failures:
        print("[manual_r01] 모든 데이터 수집 실패. 중단합니다.", file=sys.stderr)
        sys.exit(1)

    try:
        generate_report.run()
    except Exception:
        print(f"[manual_r01] generate_report 실패:\n{traceback.format_exc()}", file=sys.stderr)
        sys.exit(1)

    try:
        send_email.run(recipient=args.emails)
    except Exception:
        print(f"[manual_r01] send_email 실패:\n{traceback.format_exc()}", file=sys.stderr)
        sys.exit(1)

    if failures:
        print(f"[manual_r01] 완료 (일부 실패: {failures})")
    else:
        print("[manual_r01] 완료 (전체 성공)")
